...
\]
\begin{enumerate}[(a)]
    \item  Describe an algorithm that, given as input $G=(V,E)$ and a starting vertex $s$, finds for all vertices $v\in V$ a path from $s$ to $v$ whose bottleneck capacity is as large as possible. If for a vertex $v$ no such path exists from $s$ to $v$, the algorithm should return $d[v]=0$. Your algorithm should run in $\mathcal{O}(|V|+|E|)$ time.
